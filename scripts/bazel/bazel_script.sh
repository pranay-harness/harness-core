...
if [ "${RUN_BAZEL_TESTS}" == "true" ]; then
  ...
  bazel ${bazelrc} test --cache_test_results=no --define=${HARNESS_ARGS} --keep_going ${GCP} ${BAZEL_ARGUMENTS} -- //... -//product/... -//commons/... -//200-functional-test/... -//190-deployment-functional-tests/... || true
  exit 0
fi
