...
public class PlanExecutionServiceImpl implements PlanExecutionService {
  ...
  private void emitEvent(PlanExecution planExecution) {
    ...
    eventEmitter.emitEvent(OrchestrationEvent.newBuilder()
                               ...
                               .build());
    planStatusUpdateSubject.fireInform(PlanStatusUpdateObxxxxxxxx:onPlanStatusUpdate, ambiance);
  }
  ...
  public List<PlanExecution> findAllByAccountIdAndOrgIdAndProjectIdAndLastUpdatedAtInBetweenTimestamps(
    ...
    setupAbstractionSubFields.put(PmsEventMonitoringConstants.ACCOUNT_ID, accountId);
    setupAbstractionSubFields.put(PmsEventMonitoringConstants.ORG_ID, orgId);
    setupAbstractionSubFields.put(PmsEventMonitoringConstants.PROJECT_ID, projectId);
    Criteria criteria = new Criteria()
