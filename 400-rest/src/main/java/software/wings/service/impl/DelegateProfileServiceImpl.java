...
public class DelegateProfileServiceImpl implements DelegateProfileService, AccountCrudObserver {
  ...
  public static final String NG_PRIMARY_PROFILE_NAME_TEMPLATE = "Primary %s Configuration (%s)";
  ...
  /**
   * Profile name must be unique for the account (accountId/profileName pair is unique index)
   *
   * @param owner - Organization/Project pair, this is unique per account, so it's used to make the name unique
   * @param isNg  - if this is NG entity
   * @return - unique name for the profile on the account level
   */
  private String getProfileName(final DelegateEntityOwner owner, final boolean isNg) {
    if (isNg) {
      if (DelegateEntityOwnerHelper.isAccount(owner)) {
        return NG_PRIMARY_PROFILE_NAME_ACCOUNT;
      } else if (DelegateEntityOwnerHelper.isOrganisation(owner)) {
        return String.format(NG_PRIMARY_PROFILE_NAME_TEMPLATE, "Organization", owner.getIdentifier());
      } else {
        return String.format(NG_PRIMARY_PROFILE_NAME_TEMPLATE, "Project", owner.getIdentifier());
      }
    } else {
